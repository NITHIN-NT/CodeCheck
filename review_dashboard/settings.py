...
# During build, skip apps that require database access
# This allows collectstatic to run without SQLite
IS_BUILD = os.environ.get('VERCEL_BUILD') == '1'

if IS_BUILD:
    # Minimal apps for collectstatic - no database-dependent apps
    INSTALLED_APPS = [
...
# Minimal middleware during build
if IS_BUILD:
    MIDDLEWARE = [
...
# During build, we don't need a real database - collectstatic doesn't use it
# But Django still requires DATABASES to be defined
if IS_BUILD:
    # Use a minimal database config that won't be accessed during collectstatic
    # The backend import will still fail, but we'll handle it by monkey-patching
    DATABASES = {
        'default': {
    ...
    # Monkey-patch to prevent database connection during build
    # This allows collectstatic to run without SQLite
    import django.db
